...
with open(f'citeseer__epoch_600__0to30__PhD.json', 'r') as file:
    data = json.load(file)

    GCN = np.array(data['GCN'])
    HGAT = np.array(data['HGAT-sum'])

    GCN_mean_first_15_layers = GCN[:17].mean()
    GCN_mean_last = GCN[17:].mean()

    HGAT_mean_first_15_layers = HGAT[:10].mean()
    HGAT_mean_last = HGAT[10:].mean()
